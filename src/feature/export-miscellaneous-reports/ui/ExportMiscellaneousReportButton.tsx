...
export default function ExportMiscellaneousReportButton({
  ...
  const exportDocument = (projectReports: MiscellaneousProjectReport[]) => {
    const groupedReports = projectReports.reduce((acc, report) => {
      ...
      const facilitatorName = report.facilitator;

      if (!acc[projectName]) {
        acc[projectName] = {};
      }
      if (!acc[projectName][facilitatorName]) {
        acc[projectName][facilitatorName] = [];
      }
      acc[projectName][facilitatorName].push(report);
      return acc;
    }, {} as Record<string, Record<string, MiscellaneousProjectReport[]>>);
    ...
    const documentContent = [
      ...
      ...Object.entries(groupedReports).flatMap(
        ([projectName, facilitators]) => [
          new Paragraph({
            children: [
              new TextRun({ text: projectName, size: 24, bold: true }),
            ],
            alignment: AlignmentType.CENTER,
            spacing: { after: 200 },
          }),
          ...
          ...Object.entries(facilitators).flatMap(
            ([facilitatorName, reports]) => [
              new Paragraph({
                children: [
                  new TextRun({
                    ...
                    ...(report.praisePoints?.length
                      ? report.praisePoints.map(
                          (point) =>
                            new Paragraph({
                              children: [
                                new TextRun({ text: point, size: 14 }),
                              ],
                              bullet: { level: 1 },
                              spacing: { after: 200 },
                            })
                    ...
                    ...(report.prayerRequests?.length
                      ? report.prayerRequests.map(
                          (request) =>
                            new Paragraph({
                              children: [
                                new TextRun({ text: request, size: 14 }),
                              ],
                              bullet: { level: 1 },
                              spacing: { after: 200 },
                            })
                    ...
                    new Paragraph({
                      children: [
                        new TextRun({
                          text:
                            report.concerns ||
                            "No concerns or struggles available.",
                          size: 14,
                        }),
                      ],
                      spacing: { after: 200 },
                  ...
                  sections.push(
                    ...
                    new Paragraph({
                      children: [
                        new TextRun({
                          text:
                            report.story || "No story or testimony available.",
                          size: 14,
                        }),
                      ],
                      spacing: { after: 200 },
